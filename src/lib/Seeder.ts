...
class Seeder {
	...
	async getCount(model: any, filter: any) {
		...
		return Number((await model.find(filter).count()) || 0) > 0
			? false
			: true
	}
	...
	async runSeeder() {
		try {
			eventEmitter.emit("logging", `Migration Started!`)
			...
			for (let db of allDBs) {
				...
				models.BlackList = await dbConnection.getModel(
					blackListSchema,
					"BlackList"
				)
				...
				for (let seeder of seeders) {
					...
					let uniqueFilter = {}
					uniqueFilter[`${seeder.unique}`] =
						seeder.value[0][`${seeder.unique}`]

					isinsertQueryRequired = await this.getCount(
						models[`${seeder.document}`],
						uniqueFilter
					)
					...
					if (isinsertQueryRequired) {
						...
						if (seeder.link && seeder.link.length) {
							for (let linkData of seeder.link) {
								linkedData[`${linkData.key}`] =
									new mongoose.Types.ObjectId(
										(await this.getData(
											models[`${linkData.document}`],
											linkData.filter
										)) as string
									)
							}
			...
			eventEmitter.emit("logging", `Migration Finished`)
		} catch (error: any) {
			eventEmitter.emit(
				"logging",
